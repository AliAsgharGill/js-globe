  ...
  countries.features.forEach(country => {
    ...
    const points = flatCoordinates.map(([lon, lat]) => {
      const {
        ...
        y,
        z
      } = latLonToXYZ(lat, lon, 1); // Radius slightly larger than Earth
...
locations.forEach(location => {
  ...
  nameSprite.position.set(markerPosition.x, markerPosition.y + 0.01,
  // Adjust position of name label closer to the marker
  markerPosition.z);
...
function zoomToLocation(position) {
  ...
  }, zoomDuration).easing(TWEEN.Easing.Quadratic.Out).start();
}
